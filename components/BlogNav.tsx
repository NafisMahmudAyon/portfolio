...
const BlogNav = ({ id }: { id: number }) => {
  ...
  return (
    <div className='flex gap-3 my-4'>
      {isPrev && <Tooltip>
        <TooltipAction outline>
          <Link href={prevLink} className='flex items-center gap-2 '><ChevronLeft className='size-5' /> <span>Previous Project</span></Link>
        </TooltipAction>
      ...
      {isNext && <Tooltip>
        <TooltipAction outline>
          <Link href={nextLink} className='flex items-center gap-2 '><span>Next Project</span> <ChevronRight className='size-5' /></Link>
        </TooltipAction>
        <TooltipContent side="top" className=''>
          <p>
            {data[currentBlogId + 1]?.title}
